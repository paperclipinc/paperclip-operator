...
# paperclip.ai/paperclip-operator-bundle:$VERSION and paperclip.ai/paperclip-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= paperclip.ai/paperclip-operator
...
KIND_CLUSTER ?= paperclip-operator-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name paperclip-operator-builder
	$(CONTAINER_TOOL) buildx use paperclip-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm paperclip-operator-builder
	rm Dockerfile.cross
